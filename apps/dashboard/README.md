...
`shared/` knows nothing about *where* it runs. That keeps test-friendly logic (auth flow, data model, URL building) away from the UI layer and the Tauri APIs, and stays easy to refactor without dragging Vite / React into every change.
